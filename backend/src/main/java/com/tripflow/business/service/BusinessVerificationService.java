...
public class BusinessVerificationService {
    ...
    public BusinessVerificationResponse verify(BusinessVerificationRequest request) {
        ...
        NtsBusinessValidateRequest apiRequest = new NtsBusinessValidateRequest(List.of(
        ...
        NtsBusinessValidateResponse apiResponse = restClient.post()
        ...
        // status 자체가 없는 경우
        if (data.status() == null) {
            return new BusinessVerificationResponse(false, "사업자 상태 정보를 확인할 수 없습니다.");
        }

        // 계속사업자가 아닌 경우
        if (!"01".equals(data.status().b_stt_cd())) {
            return new BusinessVerificationResponse(false, "현재 계속사업 상태인 사업자가 아닙니다.");
        }
